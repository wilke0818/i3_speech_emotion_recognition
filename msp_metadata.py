...
parser.add_argument("--labels_file", help="Path to the labels csv", default="/om2/scratch/Wed/fabiocat/MSP/Labels/labels_consensus.json")
...
for audio in audio_iterator:
  ...
  try:  
    ...
    data[audio['Split_Set']].append([path, actor, emotion, gender, age])
    data['metadata'].append([path, actor, emotion, gender, age])
...
for subclass in data.keys():
  df = pd.DataFrame(data[subclass], columns=['path', 'actor', 'class_id', 'gender', 'age']) 
